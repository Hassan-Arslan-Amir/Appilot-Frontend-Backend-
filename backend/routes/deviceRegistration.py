...
def schedule_notification(task, device_names, start_time, end_time, time_zone, job_id):
    ...
    try:
        ...
        if task_inputs:
            inputs_list = []
            for input_item in task_inputs:
                if isinstance(input_item, dict):
                    input_name = input_item.get("inputName", "Unknown Input")
                    input_value = input_item.get("inputValue", "")
                    if input_value:
                        inputs_list.append(f"{input_name}: {input_value}")
                    else:
                        inputs_list.append(input_name)
            inputs_text = ", ".join(inputs_list) if inputs_list else "None"
